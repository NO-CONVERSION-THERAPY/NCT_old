...
function normalizePublicMapDataUrl(value, fallback = '/content/map-data.json') {
  const normalizedValue = readTrimmedEnvValue(value).replace(/\/+$/, '');

  if (!normalizedValue) {
    return fallback;
  }

  const retiredRuntimeUrls = new Set([
    '/api/map-data',
    'https://nct.hosinoeiji.workers.dev/api/map-data',
    'https://www.victimsunion.org/api/map-data'
  ]);

  if (retiredRuntimeUrls.has(normalizedValue)) {
    return fallback;
  }
...
// 迁移后前端直接读取公开 JSON 地图源，后端不再承接地图聚合接口。
const publicMapDataUrl = normalizePublicMapDataUrl(process.env.PUBLIC_MAP_DATA_URL);
...
const apiUrl = String(publicMapDataUrl || '').trim();
const nctBackendServiceUrl = readTrimmedEnvValue(
  process.env.NCT_BACKEND_SERVICE_URL,
  process.env.BACKEND_SERVICE_URL
).replace(/\/+$/, '');
const nctBackendServiceToken = readTrimmedEnvValue(
  process.env.NCT_BACKEND_SERVICE_TOKEN,
  process.env.BACKEND_SERVICE_TOKEN
);
const nctBackendServiceTimeoutMs = parsePositiveInteger(
  process.env.NCT_BACKEND_SERVICE_TIMEOUT_MS || process.env.BACKEND_SERVICE_TIMEOUT_MS,
  15000
);
