...
function extractConfirmationStateFromHtml(body) {
  const bootstrap = extractBootstrapFromHtml(body);
  const { confirmationPayload, confirmationToken } = bootstrap.pageProps || {};

  assert.ok(confirmationPayload, 'Expected confirmationPayload in React pageProps');
  assert.ok(confirmationToken, 'Expected confirmationToken in React pageProps');
...
function buildValidEnhancedSubmissionBody(overrides = {}) {
  const payload = {
    identity: '受害者本人',
    birth_year: '2008',
    sex: '男性',
...
test('root page renders successfully', async () => {
  ...
  assert.equal(bootstrap.currentPath, '/');
  assert.equal(bootstrap.apiUrl, '/content/map-data.json');
  assert.deepEqual(bootstrap.pageProps, { apiUrl: '/content/map-data.json' });
  assert.match(response.body, /\/react-app\/app\.js\?v=/);
  assert.match(response.body, /\/react-app\/app\.css\?v=/);
  assert.doesNotMatch(response.body, /window\.API_URL = "/);
});
...
test('map page renders the record container and lazy-load sentinel', async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(bootstrap.pageType, 'frontend-router');
  assert.equal(bootstrap.currentPath, '/map');
  assert.equal(bootstrap.apiUrl, '/content/map-data.json');
  assert.match(response.body, /\/react-app\/app\.js\?v=/);
  assert.doesNotMatch(response.body, /id="data-container"/);
  assert.doesNotMatch(response.body, /\/js\/map_record_stats\.js/);
});
...
test('map record page renders the standalone submission detail shell', async () => {
  ...
  assert.equal(bootstrap.currentPath, '/map/record/test-record-token?lang=en&search=test');
  assert.equal(bootstrap.apiUrl, '/content/map-data.json');
  assert.doesNotMatch(response.body, /\/js\/map_record_page\.js/);
});
...
test('map page ignores ASSET_VERSION=0 and falls back to a real cache-busting version', async () => {
  await withMockedDate('2026-04-07T12:45:00.000Z', async () => {
    ...
    assert.equal(response.statusCode, 200);
    assert.equal(bootstrap.assetVersion, '1775565900000');
    assert.match(response.body, /\/react-app\/app\.js\?v=1775565900000/);
    assert.match(response.body, /\/react-app\/app\.css\?v=1775565900000/);
...
test('map page ignores app locals assetVersion=0 and rewrites it before rendering', async () => {
  await withMockedDate('2026-04-07T12:46:00.000Z', async () => {
    ...
    assert.equal(response.statusCode, 200);
    assert.equal(bootstrap.assetVersion, '1775565960000');
    assert.match(response.body, /\/react-app\/app\.js\?v=1775565960000/);
    assert.match(response.body, /\/react-app\/app\.css\?v=1775565960000/);
    assert.doesNotMatch(response.body, /window\.ASSET_VERSION = "0"/);
  });
});
...
test('form page includes school name and address autocomplete hooks', async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(bootstrap.pageType, 'frontend-router');
  assert.equal(bootstrap.currentPath, '/form');
  assert.equal(bootstrap.apiUrl, '/content/map-data.json');
  assert.equal(bootstrap.i18n.form.sections.basic, '个人基本信息');
  assert.equal(bootstrap.i18n.form.fields.schoolName, '机构名称');
  assert.equal(bootstrap.i18n.form.buttons.openMap, '点击可直接在地图上选点');
  assert.equal(bootstrap.i18n.form.buttons.getCurrentLocation, '获取当前位置');
  assert.match(response.body, /\/react-app\/app\.js\?v=/);
  assert.doesNotMatch(response.body, /name="birth_year"/);
  assert.doesNotMatch(response.body, /name="form_token"/);
  assert.doesNotMatch(response.body, /\/js\/form_api\.js/);
});

test('form page recomputes birth year options for long-lived runtimes', async () => {
  const runtimeConfigUrl = pathToFileURL(path.join(projectRoot, 'frontend/src/formRuntimeConfig.js')).href;
  const { buildLocalizedFormConfig } = await import(runtimeConfigUrl);

  await withMockedDate('2025-12-31T23:30:00Z', async () => {
    const formConfig = buildLocalizedFormConfig({});
    assert.equal(formConfig.formRules.birthYear.max, 2025);
  });

  await withMockedDate('2026-01-01T00:30:00Z', async () => {
    const formConfig = buildLocalizedFormConfig({});
    assert.equal(formConfig.formRules.birthYear.max, 2026);
  });
...
test('static area selector payload is served from public content', async () => {
  const app = loadApp({ DEBUG_MOD: 'false' });
  const response = await requestPath(app, '/content/area-selector.json');
  const payload = JSON.parse(response.body);

  assert.equal(response.statusCode, 200);
  assert.match(response.headers['content-type'], /application\/json/);
  assert.ok(Array.isArray(payload.provincesByLanguage['zh-CN']));
  assert.equal(payload.provincesByLanguage.en[0].code, '110000');
  assert.equal(payload.citiesByProvinceCode['110000'][0].code, '110101');
  assert.equal(payload.countiesByCityCode['350500'][0].code, '350502');
});

test('static map payload is served from public content for both frontends', async () => {
  const app = loadApp({ DEBUG_MOD: 'false' });
  const response = await requestPath(app, '/content/map-data.json');
  const payload = JSON.parse(response.body);

  assert.equal(response.statusCode, 200);
  assert.match(response.headers['content-type'], /application\/json/);
  assert.ok(Number.isFinite(payload.schoolNum));
  assert.ok(Number.isFinite(payload.formNum));
  assert.ok(Array.isArray(payload.data));
  assert.ok(payload.data.length > 100);
  assert.ok(['google-script', 'public-map-data'].includes(payload.source));
});

test('retired area options API returns guidance for the static payload', async () => {
  const app = loadApp({ DEBUG_MOD: 'false' });
  const response = await requestPath(app, '/api/area-options?provinceCode=110000&lang=zh-CN');

  assert.equal(response.statusCode, 410);
  assert.match(response.headers['content-type'], /application\/json/);
  assert.match(response.body, /area options api has been retired/i);
  assert.match(response.body, /content\/area-selector\.json/);
});
...
test('debug page renders when debug mode is enabled', async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(bootstrap.pageType, 'debug');
  assert.equal(bootstrap.pageProps.debugTools.length, 5);
  assert.deepEqual(
    bootstrap.pageProps.debugTools.map((tool) => tool.href),
    [
      '/debug/submit-error',
      '/debug/submit-preview',
      '/debug/submit-confirm',
      '/debug/correction-submit-success',
      '/debug/correction-submit-error'
    ]
  );
  assert.equal(bootstrap.pageProps.debugSections[1].title, '站点配置');
});
...
test('debug page respects explicit language selection', async () => {
  ...
  const englishBootstrap = extractBootstrapFromHtml(englishResponse.body);
  const traditionalChineseBootstrap = extractBootstrapFromHtml(traditionalChineseResponse.body);

  assert.equal(englishResponse.statusCode, 200);
  assert.match(englishResponse.body, /<html lang="en">/);
  assert.equal(englishBootstrap.lang, 'en');
  assert.equal(englishBootstrap.pageProps.debugSections[1].title, 'Site Configuration');
  assert.equal(englishBootstrap.pageProps.debugTools[0].label, 'Open submission error preview');
  ...
  assert.equal(traditionalChineseBootstrap.lang, 'zh-TW');
  assert.equal(traditionalChineseBootstrap.pageProps.debugSections[1].title, '站點配置');
  assert.equal(traditionalChineseBootstrap.pageProps.debugTools[0].label, '查看提交失敗頁預覽');
...
test('standalone submit error preview renders a prefilled Google Form link when debug mode is enabled', async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(bootstrap.pageType, 'submit-error');
  assert.match(bootstrap.pageProps.fallbackUrl, /viewform\?usp=pp_url&entry\.842223433=11/);
  assert.match(bootstrap.pageProps.fallbackUrl, /entry\.1422578992=%E7%94%B7/);
});
...
test('standalone submit preview debug route renders mock preview data when debug mode is enabled', async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(bootstrap.pageType, 'submit-preview');
  assert.equal(bootstrap.pageProps.backFormUrl, '/debug');
  assert.match(bootstrap.pageProps.encodedPayload, /^entry\./);
  assert.ok(bootstrap.pageProps.fields.some((field) => field.value === '调试预览学院'));
  assert.ok(bootstrap.pageProps.fields.some((field) => field.value === 'debug-preview@example.com'));
});
...
test('standalone submit confirm debug route renders mock confirmation data and supports simulated submit', async () => {
  ...
  const getBootstrap = extractBootstrapFromHtml(getResponse.body);

  assert.equal(getResponse.statusCode, 200);
  assert.equal(getBootstrap.pageType, 'submit-confirm');
  assert.equal(getBootstrap.pageProps.backFormUrl, '/debug');
  assert.equal(getBootstrap.pageProps.confirmAction, '/debug/submit-confirm');
  assert.ok(getBootstrap.pageProps.fields.some((field) => field.value === '调试预览学院'));
  ...
  const postBootstrap = extractBootstrapFromHtml(postResponse.body);

  assert.equal(postResponse.statusCode, 200);
  assert.equal(postBootstrap.pageType, 'submit-success');
  assert.equal(postBootstrap.pageProps.showSubmissionDiagnostics, true);
  assert.ok(postBootstrap.pageProps.submissionDiagnostics.successfulTargets.some((target) => target.id === 'google'));
});
...
test('correction success and error debug routes render mock result pages when debug mode is enabled', async () => {
  ...
  const successResponse = await requestPath(app, '/debug/correction-submit-success');
  const errorResponse = await requestPath(app, '/debug/correction-submit-error');
  const successBootstrap = extractBootstrapFromHtml(successResponse.body);
  const errorBootstrap = extractBootstrapFromHtml(errorResponse.body);

  assert.equal(successResponse.statusCode, 200);
  assert.equal(successBootstrap.pageType, 'correction-success');
  assert.equal(successBootstrap.pageProps.showSubmissionDiagnostics, true);
  assert.ok(successBootstrap.pageProps.submissionDiagnostics.successfulTargets.some((target) => target.id === 'google'));
  assert.ok(successBootstrap.pageProps.submissionDiagnostics.failedTargets.some((target) => target.id === 'd1'));

  assert.equal(errorResponse.statusCode, 200);
  assert.equal(errorBootstrap.pageType, 'correction-error');
  assert.equal(errorBootstrap.pageProps.backFormUrl, '/debug');
  assert.equal(errorBootstrap.pageProps.showSubmissionDiagnostics, true);
  assert.match(errorBootstrap.pageProps.fallbackUrl, /entry\.270706445=/);
  assert.match(errorBootstrap.pageProps.fallbackUrl, /entry\.302336209=/);
});
...
test('correction success and error debug routes respect explicit language selection', async () => {
  ...
  const englishBootstrap = extractBootstrapFromHtml(englishSuccessResponse.body);
  const traditionalChineseBootstrap = extractBootstrapFromHtml(traditionalChineseErrorResponse.body);

  assert.equal(englishSuccessResponse.statusCode, 200);
  assert.match(englishSuccessResponse.body, /<html lang="en">/);
  assert.equal(englishBootstrap.lang, 'en');
  assert.equal(englishBootstrap.pageType, 'correction-success');
  assert.equal(englishBootstrap.i18n.submitStatus.title, 'Debug Submission Result');
  ...
  assert.equal(traditionalChineseBootstrap.lang, 'zh-TW');
  assert.equal(traditionalChineseBootstrap.pageType, 'correction-error');
  assert.equal(traditionalChineseBootstrap.i18n.institutionCorrection.errorPage.openFallback, '打開 Google Form 繼續提交');
  assert.match(traditionalChineseBootstrap.pageProps.fallbackUrl, /entry\.270706445=/);
...
test('privacy page documents the language cookie, form-disclosure flow, and footer exposes the link', async () => {
  ...
  const rootBootstrap = extractBootstrapFromHtml(rootResponse.body);
  const privacyBootstrap = extractBootstrapFromHtml(privacyResponse.body);

  assert.equal(rootResponse.statusCode, 200);
  assert.equal(rootBootstrap.i18n.navigation.privacy, '隐私政策 / Cookie 说明');

  assert.equal(privacyResponse.statusCode, 200);
  assert.equal(privacyBootstrap.pageType, 'frontend-router');
  assert.equal(privacyBootstrap.lang, 'en');
  assert.equal(privacyBootstrap.i18n.privacy.title, 'Privacy & Cookie Notice');
  assert.equal(privacyBootstrap.i18n.privacy.sections.formSubmission, 'Form Submission And Public Display');
  assert.equal(privacyBootstrap.i18n.privacy.sections.thirdParty, 'Third-Party Services');
  assert.equal(privacyBootstrap.i18n.privacy.sections.retention, 'Retention And Removal');
  assert.equal(privacyBootstrap.i18n.privacy.sections.publicInfo, 'Public Data');
  assert.equal(privacyBootstrap.i18n.privacy.cookieFields.name, 'Name');
  assert.equal(privacyBootstrap.i18n.privacy.cookieValues.attributes, 'SameSite=Lax.');
  assert.equal(privacyBootstrap.i18n.privacy.cookieValues.retention, 'Up to 30 days (Max-Age=2592000).');
  assert.ok(privacyBootstrap.i18n.privacy.formItems.some((item) => /browser will request location permission/i.test(item)));
...
test('blog list shows translated titles when english language is selected', async () => {
  ...
  try {
    ...
    const indexResponse = await requestPath(app, '/content/blog/index.json');
    const bootstrap = extractBootstrapFromHtml(response.body);
    const blogIndex = JSON.parse(indexResponse.body);
    const originalTitle = '關於心種子教育違法辦學的控告';
    const articleEntry = blogIndex.entries.find((entry) => entry.title === originalTitle);
    ...
    assert.equal(articleEntry.Language, '正體中文');
    assert.equal(articleEntry.CreationDate, '2026年3月13日');
    assert.deepEqual(articleEntry.tagid, ['5']);
    assert.equal(bootstrap.i18n.blog.articleLanguages.zhTW, 'Traditional Chinese');
    assert.equal(bootstrap.i18n.blog.tags['5'], 'Law');
  } finally {
    restoreFetch();
    clearProjectModules();
  }
...
test('blog article shows bilingual content when english language is selected', async () => {
  ...
  try {
    ...
    const articleResponse = await requestPath(app, `/content/blog/articles/${articleId}.json`);
    const bootstrap = extractBootstrapFromHtml(response.body);
    ...
    assert.equal(bootstrap.pageType, 'frontend-router');
    assert.equal(bootstrap.lang, 'en');
    assert.equal(articlePayload.articleId, '關於心種子教育違法辦學的控告');
    assert.match(articlePayload.articleHtml, /blog-bilingual-block--heading/);
    assert.match(articlePayload.articleHtml, /data-blog-translation-source=/);
    assert.match(articlePayload.articleHtml, /关于山东心种子教育咨询有限公司非法限制人身自由及身心摧残的维权通告/);
  } finally {
    restoreFetch();
    clearProjectModules();
  }
...
test('submit route still accepts a valid protected form in dry run mode', async () => {
  ...
  assert.doesNotMatch(response.body, /<meta name="robots"/i);
  assert.equal(bootstrap.pageType, 'submit-preview');
  assert.equal(bootstrap.pageProps.backFormUrl, '/form');
  assert.ok(bootstrap.pageProps.fields.some((field) => field.entryId === 'entry.5034928' && field.value === '测试机构'));
  assert.ok(bootstrap.pageProps.fields.some((field) => field.entryId === 'entry.842223433' && field.label === '出生年份' && field.value === expectedAge));
  assert.ok(bootstrap.pageProps.fields.some((field) => field.entryId === 'entry.1422578992' && field.label === '性别' && field.value === '男'));
  clearProjectModules();
});
...
test('submit route accepts agent submissions with MtF selected for other gender identity in dry run mode', async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(bootstrap.pageType, 'submit-preview');
  assert.ok(bootstrap.pageProps.fields.some((field) => field.entryId === 'entry.842223433' && field.label === '受害者出生年份' && field.value === expectedAge));
  assert.ok(bootstrap.pageProps.fields.some((field) => field.entryId === 'entry.1422578992' && field.label === '受害者性别' && field.value === 'MtF'));
  clearProjectModules();
});
...
test('submit route accepts custom other gender identity text in dry run mode', async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(bootstrap.pageType, 'submit-preview');
  assert.ok(bootstrap.pageProps.fields.some((field) => field.entryId === 'entry.1422578992' && field.label === '性别' && field.value === '__other_option__'));
  assert.ok(bootstrap.pageProps.fields.some((field) => field.entryId === 'entry.1422578992.other_option_response' && field.label === '性别' && field.value === '非二元'));
  clearProjectModules();
});
...
test('submit route accepts Queer selected for other gender identity in dry run mode', async () => {
  ...
  assert.equal(bootstrap.pageType, 'submit-preview');
  assert.ok(bootstrap.pageProps.fields.some((field) => field.entryId === 'entry.1422578992' && field.label === '性别' && field.value === '__other_option__'));
  assert.ok(bootstrap.pageProps.fields.some((field) => field.entryId === 'entry.1422578992.other_option_response' && field.label === '性别' && field.value === 'Queer'));
  clearProjectModules();
});
...
test('standalone submit route renders a dry run preview page when FORM_DRY_RUN=true', async () => {
  ...
  const response = await requestApp(app, {
    ...
    body: buildValidEnhancedSubmissionBody({
      form_token: issueFormProtectionToken({
        secret: 'test-form-protection-secret',
        issuedAt: Date.now() - 5000
      })
...
test('submit route renders a confirmation page before sending to Google Form in normal mode', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(bootstrap.pageProps.backFormUrl, '/form');
    assert.equal(bootstrap.pageProps.confirmAction, '/submit/confirm');
    assert.ok(bootstrap.pageProps.confirmationToken);
    assert.ok(bootstrap.pageProps.confirmationPayload);
    assert.ok(bootstrap.pageProps.fields.some((field) => field.label === '机构名称' && field.value === '测试机构'));
    assert.ok(bootstrap.pageProps.fields.some((field) => field.label === '机构联系方式' && field.value === 'test@example.com'));
    restore();
  } finally {
    clearProjectModules();
  }
...
test('submit confirm route sends the reviewed payload to Google Form in normal mode', { concurrency: false }, async () => {
  ...
  try {
    ...
    const { confirmationPayload, confirmationToken } = extractConfirmationStateFromHtml(reviewResponse.body);
    const confirmResponse = await requestApp(app, {
      path: '/submit/confirm',
      method: 'POST',
      headers: {
    ...
    const confirmBootstrap = extractBootstrapFromHtml(confirmResponse.body);
    ...
    assert.equal(confirmBootstrap.pageType, 'submit-success');
    restore();
  } finally {
    clearProjectModules();
  }
...
test('submit confirm route returns success when D1 succeeds but Google Form fails in both mode', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(confirmResponse.statusCode, 200);
    assert.equal(confirmBootstrap.pageType, 'submit-success');
    assert.equal(confirmBootstrap.pageProps.showSubmissionDiagnostics, true);
    assert.ok(confirmBootstrap.pageProps.submissionDiagnostics.successfulTargets.some((target) => target.id === 'd1'));
    assert.ok(confirmBootstrap.pageProps.submissionDiagnostics.failedTargets.some((target) => target.id === 'google' && /google form unavailable/.test(target.error)));
    restore();
  } finally {
    clearProjectModules();
  }
...
test('submit confirm route renders a prefilled Google Form fallback link when upstream submission fails', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(confirmResponse.statusCode, 500);
    assert.equal(confirmBootstrap.pageType, 'submit-error');
    assert.match(confirmBootstrap.pageProps.fallbackUrl, /viewform\?usp=pp_url&entry\.842223433=/);
    assert.match(confirmBootstrap.pageProps.fallbackUrl, /entry\.5034928=%E6%B5%8B%E8%AF%95%E6%9C%BA%E6%9E%84/);
    assert.match(confirmBootstrap.pageProps.fallbackUrl, /entry\.500021634=%E5%8F%97%E5%AE%B3%E8%80%85%E6%9C%AC%E4%BA%BA/);
    restore();
  } finally {
    clearProjectModules();
  }
...
test('submit confirm route renders failure page when Google Form and D1 both fail in both mode', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(confirmBootstrap.pageProps.showSubmissionDiagnostics, true);
    assert.ok(confirmBootstrap.pageProps.submissionDiagnostics.failedTargets.some((target) => target.id === 'google' && /google form unavailable/.test(target.error)));
    assert.ok(confirmBootstrap.pageProps.submissionDiagnostics.failedTargets.some((target) => target.id === 'd1' && /d1 unavailable/.test(target.error)));
    assert.match(confirmBootstrap.pageProps.fallbackUrl, /viewform\?usp=pp_url&entry\.842223433=/);
    restore();
  } finally {
    clearProjectModules();
...
test('submit confirm route rejects tampered confirmation payloads', async () => {
  ...
  const confirmResponse = await requestApp(app, {
    ...
    body: new URLSearchParams({
      confirmation_token: confirmationToken,
      confirmation_payload: `${confirmationPayload}tampered`
    }).toString()
  });
...
test('retired map API keeps CORS enabled while translate API stays same-origin only', async () => {
  const app = loadApp({ DEBUG_MOD: 'false' });
  ...
  assert.equal(mapResponse.statusCode, 410);
  ...
  assert.match(mapResponse.body, /retired/i);
});

test('retired map API returns guidance even when callers repeat cross-origin reads', async () => {
  const app = loadApp({ DEBUG_MOD: 'false' });
  ...
  assert.equal(firstResponse.statusCode, 410);
  assert.equal(firstResponse.headers['access-control-allow-origin'], '*');
  assert.equal(secondResponse.statusCode, 410);
  assert.equal(secondResponse.headers['access-control-allow-origin'], '*');
  assert.match(secondResponse.body, /public json source/i);
});
...
test('retired map API keeps returning guidance when refresh is requested', async () => {
  const app = loadApp({ DEBUG_MOD: 'false' });
  const response = await requestApp(app, {
  ...
  assert.equal(response.statusCode, 410);
  assert.match(response.body, /public json source/i);
});
