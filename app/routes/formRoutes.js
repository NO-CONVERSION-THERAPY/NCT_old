...
function registerSubmissionFlowRoutes({
  ...
  router.post(flow.submitPath, submitLimiter, async (req, res) => {
    ...
    if (isNctBackendServiceConfigured()) {
      ...
      try {
        const submissionResult = await prepareFormSubmission({
          body: req.body,
          req
        });
        encodedPayload = submissionResult.encodedPayload || '';
        const fields = buildGoogleFormFields(submissionResult.values, req.t);
        const confirmationFields = buildConfirmationFields(submissionResult.values, req.t);

        if (submissionResult.mode === 'preview') {
        ...
        return renderFormFlowPage({
          ...
          legacyData: {
            backFormUrl: flow.backFormUrl,
            confirmationPayload: submissionResult.confirmationPayload,
            confirmationToken: submissionResult.confirmationToken,
        ...
        if (error.statusCode === 400) {
          ...
          if (details.length > 0) {
            ...
            return res.status(400).send(`${req.t('server.submitFailedPrefix')}${details.join('；')}`);
          }

          logAuditEvent(req, 'submit_protection_failed', {
            flow: flow.submitPath,
            reason: error.payload && error.payload.reason ? error.payload.reason : 'invalid_token',
            status: 400
          });
        ...
        console.error('Submission Proxy Error:', error.message);
  ...
  router.post(flow.confirmPath, confirmLimiter, async (req, res) => {
    ...
    if (isNctBackendServiceConfigured()) {
      ...
      try {
        const submissionResult = await confirmFormSubmission({
          confirmationPayload: String(req.body.confirmation_payload || '').trim(),
          confirmationToken: String(req.body.confirmation_token || '').trim()
        });
        encodedPayload = submissionResult.encodedPayload || '';
        ...
        if (!Array.isArray(submissionResult.successfulTargets) || submissionResult.successfulTargets.length === 0) {
          logAuditEvent(req, 'submit_failed', {
            ...
            flow: flow.confirmPath,
            status: submissionResult.statusCode || 500
          });
          return renderSubmitFailurePage({
            encodedPayload,
            flow,
